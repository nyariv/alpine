...
test('Promises in x-data works correctly', async () => {
    document.body.innerHTML = `
        <div x-data="{ foo: 'bar', bar: new Promise((resolve, reject) => {resolve('baz');}) }">
            <span x-text="foo"></span>
            <button x-on:click="bar.then(val => {foo = val})"></button>
        </div>
    `
    Alpine.start()
